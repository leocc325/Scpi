...
***优化点: 可以在节点中保存当前节点指令的起始字符,然后根据起始字符按顺序对子节点排序(std::set)，查找时直接根据输入指令的起始字母直接定位到排好序的节点附近，比如节点本身是S开头的，输入指令也是S开头，就直接跳转到S开头的子节点处遍历这一群s开头的节点，这样可以跳过大量无需匹配的节点，无需挨个匹配
这里是两个目标函数用于测试
```c++
